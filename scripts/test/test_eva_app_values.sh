...
if rg -q 'kubectl delete (secret|pvc|deployment|statefulset)' "$REPO_ROOT/src/solution/roles/eva_app/tasks/main.yaml"; then
  echo 'EVA App TLS Job cleanup deletes a protected resource type' >&2
  exit 1
fi

rg -Fq 'eva_iam_app_handoff_path' "$REPO_ROOT/src/solution/roles/eva_iam/tasks/main.yaml"
rg -Fq 'eva_persistent_state_root' "$REPO_ROOT/src/solution/roles/eva_iam/tasks/main.yaml"
rg -Fq 'mode: "0700"' "$REPO_ROOT/src/solution/roles/eva_iam/tasks/main.yaml"
rg -Fq 'mode: "0600"' "$REPO_ROOT/src/solution/roles/eva_iam/tasks/main.yaml"
if rg -Fq "eva_site_config_root ~ '/' ~ eva_app_values_host ~ '/eva-iam.yaml'" "$REPO_ROOT/src/solution/roles/eva_app/tasks/main.yaml"; then
