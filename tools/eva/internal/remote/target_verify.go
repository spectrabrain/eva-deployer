...
func runAskpassSSH(ctx context.Context, args []string, password string) error {
	directory, err := os.MkdirTemp("/run/eva", "askpass-")
	if err != nil {
		return err
	}
	...
	command.ExtraFiles = []*os.File{reader} // fd 3, consumed only by askpass.
	command.Env = append(os.Environ(), "SSH_ASKPASS="+helper, "SSH_ASKPASS_REQUIRE=force", "DISPLAY=eva")
	if output, err := command.CombinedOutput(); err != nil {
		_ = output // SSH diagnostics can include infrastructure details; never print them.
