...
eva install ./eva-release-3.2.0 --component agent --values agent=./agent-fix.yaml --set replicaCount=2
...
eva verify
eva shell
eva shell --workspace /etc/eva/sites/customer-a --release /opt/eva/releases/3.2.0
eva shell --runtime-root /opt/eva/runtime --command 'helm version'
...
| `retry [--yes] [operation-id]` | 있음 | 실패 Operation의 저장 Plan을 새 Operation으로 복제하고 적용 |
| `status [operation-id]` | 없음 | 최신 또는 지정 operation 상태 출력 |
| `troubleshoot apt` | 없음 | APT repository validation 진단 |
| `troubleshoot apt --fix-known --yes` | 있음 | 명시적으로 지원하는 APT repository 복구 적용 |
...
`eva retry --yes [operation-id]`는 최신 또는 지정 `failed` Operation만 받을 수 있다. 원본 Operation과 결과·로그는 보존하고, 저장된 Plan 및 staged override input을 새 `planned` Operation으로 복제한 뒤 즉시 apply한다. 새 record의 `source_operation_id`와 원본 record의 `retry_operation_id`로 관계를 기록한다. 현재 Workspace나 Release 입력을 다시 읽어 Plan을 재계산하지 않으므로, 재시도 대상은 원본에서 승인된 입력으로 고정된다.

Cloud Infra Plan의 `eva apply`와 `eva retry`는 Operation을 `running`으로 전환하기 전에 control node에서 `apt-get update`를 실행해 APT repository 상태를 진단한다. Jenkins LTS 2026 key migration의 `NO_PUBKEY 7198F4B714ABFC68`은 지원되는 known problem이다. EVA는 repository URL, 변경 파일과 위험을 보여 준 뒤 interactive `yes`를 별도로 받아야만 `/etc/apt/keyrings/jenkins-keyring.asc`와 `/etc/apt/sources.list.d/jenkins.list`를 수정한다. `apply --yes`는 이 동의로 해석하지 않는다. 비대화형 apply는 수정 없이 중단하며, 운영자는 `eva troubleshoot apt`로 진단하거나 `eva troubleshoot apt --fix-known --yes`로 명시적으로 복구할 수 있다. 복구는 기존 source와 keyring을 transaction backup으로 보관하고, 공식 key의 full fingerprint `5E386EADB55F01504CAE8BCF7198F4B714ABFC68` 검증, `signed-by` source 정규화, APT 재검증 순서로 수행한다. 재검증 실패 시 두 파일을 복원하고 중단한다.

`eva install [release-path] --workspace <path> [--component <name>]`는 local Release source를 검증하고, 필요하면 `release prepare`, Workspace 검증, Plan 요약, 확인, operation 생성, Apply를 차례로 수행하는 편의 명령이다. `--component`의 선택 규칙은 `eva plan`과 같으며, 생성된 operation에 선택 범위가 고정된다. Helm override가 있으면 summary에 `[WARN] field override detected`를 표시하고 `eva status`에도 해당 operation의 override 여부를 표시한다. 이미 준비된 `/opt/eva/releases/<version>`을 입력하면 재추출하지 않는다. Airgap Bundle 경로는 nested artifact cache import를 먼저 수행한다. TTY가 아닌 자동화에서는 `--yes`가 필수이며, 사용자가 취소하거나 Runtime/Workspace 검증에 실패하면 Release prepare와 operation 생성은 실행하지 않는다. Release tag와 positional Component shortcut은 아직 지원하지 않는다.
...
`eva shell [--runtime-root <path>] [--site <id> | --workspace <path>] [--release <path>]`은 검증된 Managed Runtime 도구의 디렉터리를 기존 `PATH` 앞에 추가해 shell을 시작한다. 항상 `EVA_RUNTIME_ROOT`를 설정하고, Workspace를 지정하면 `EVA_SITE_ID`, `EVA_WORKSPACE_ROOT`를, Release를 지정하면 `EVA_RELEASE_ROOT`, `EVA_RELEASE_VERSION`, `EVA_REPO_ROOT`를 설정한다. 기본 shell은 `$SHELL`이며 없으면 `/bin/bash`를 사용한다. `--shell`은 절대 경로의 실행 가능한 regular file만 허용하고, `--command`는 자동화 또는 확인을 위해 선택 shell의 `-lc`로 한 명령을 실행한다. shell 내부의 raw 명령은 operation state나 operation log에 기록되지 않으므로, CLI가 관리하는 설치 상태와 달라질 수 있다는 경고를 시작 전에 반드시 표시한다.

`eva-offline_<version>_ubuntu24.04_amd64.tar.gz`는 완성된 `runtime/` subtree를 포함해야 한다. 이 subtree에는 `runtime.yaml`, `venv/bin/ansible-playbook`, `bin/helm`, `bin/kubectl`, `bin/kustomize`, `bin/oras`가 들어간다. archive의 나머지 wheel, deb, image, model 자산은 Runtime bootstrap에서 추출하지 않는다. `eva runtime bootstrap --offline <archive>`는 `runtime/` subtree만 staging에 안전하게 추출해 검증·publish하며, 경로 탈출, symlink/hardlink, 특수 파일은 거부한다. `eva install`은 `repository.mode: local`이고 Runtime이 없을 때 원본 Release 또는 Airgap Bundle에서 `eva-offline` artifact를 자동 bootstrap한다. Cloud/Remote online bootstrap은 `eva runtime bootstrap`으로 수행한다.
...
Remote Runtime은 `eva runtime bootstrap`으로 online bootstrap을 먼저 수행한다. 명시적으로 Offline payload를 선택한 경우에는 `eva runtime bootstrap --offline <archive>`를 사용한다. 현재 `eva exec`는 검증된 Runtime descriptor의 allowlist 도구만 직접 실행한다. `eva shell`은 Runtime PATH와 선택한 Workspace/Release 환경을 제공하지만, shell의 raw 실행은 operation log에 기록하지 않는다. 추후 raw 실행 audit을 추가할 때는 실행 사용자, 명령 시작/종료 시각, 종료 코드만 기록하고 argument 값과 stdout/stderr는 1차에서 수집하지 않는다.
